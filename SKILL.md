...
- Architecture, flowers, objects, portraits, and editorial subjects that benefit from refined matte cardstock, folded cut-paper planes, restrained dimensional layering, and integrated micro-typography: `SCHEME_EDITORIAL_LAYERED_PAPERCUT`.
...
1. Create a single portrait **style selector**: show the unaltered source image at the top and fourteen small stamp thumbnails below in a balanced grid with equal-size tiles. Include every visible scheme: `1 CHIN_PAPERCUT`, `2 INK_WASH`, `3 TRAV_VINT`, `4 CLEAR_LINE_ADVENTURE`, `5 ART_DECO_MUSEUM_POSTER`, `6 MINI_ICON`, `7 SURR_POP`, `8 RED_PICTOGRAM`, `9 ORNATE_LINEWORK`, `10 NAIVE_WATERCOLOR_BESTIARY`, `11 BLUE_PENCIL_FIELD_SKETCH`, `12 CALM_SOLID_WASH_DOODLE`, `13 VINTAGE_MIXED_MEDIA_COLLAGE`, and `14 EDITORIAL_LAYERED_PAPERCUT`. Use clean, high-contrast labels outside each thumbnail. Keep every tile equal in status; use a 4×4 grid with two intentional empty cells or another balanced layout. This selector is a visual direction board, not the final stamp; keep text inside thumbnail stamps minimal. Keep `SCHEME_CHIN_WOOD` hidden unless the user explicitly requests it by name.
